...
export default function Page() {
	...
	const { search, setSearch } = useNavigationSearch({
		searchBarOptions: {
			placeholder: 'Procurar Exportadores',
		},
	})
	...
	const {
		data: exporters,
		isLoading: isExportersLoading,
		error: exportersError,
		isError: isExportersError,
	} = useQueryMany<{
		id: string
		surname: string
		other_names: string
		...
		WHERE ac.subcategory LIKE '%${MultiCategory.TRADER_EXPORT}%'
	...
	const filteredExporters = useMemo(() => {
		if (!search && !newSearchKey) return exporters

		if (newSearchKey) {
			return exporters.filter((trader) => trader.admin_post_id?.toLowerCase().includes(newSearchKey.toLowerCase()))
		}
					...
					<FlatList
						...
						ListEmptyComponent={() => (
							<View className="flex-1 items-center justify-center h-[400px]">
								<EmptyPlaceholder message="Não há exportadores para mostrar" />
							</View>
						)}
					/>
				)}
